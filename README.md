...
Dado el límite de tiempo de la prueba, opté por desarrollar la solución en **React**, tecnología listada como "deseable" en el perfil requerido.
...
* **Tailwind CSS:** Utilizado para un desarrollo rápido de UI. Permitió construir una interfaz responsiva y moderna, manteniendo un paquete CSS final sumamente ligero (ideal para usuarios con conexiones inestables).
...
1.  **Scroll Infinito (IntersectionObserver):** En lugar de renderizar todo el DOM de golpe o usar una paginación tradicional, la cuadrícula carga los cursos progresivamente conforme el usuario hace *scroll*, logrando un flujo de exploración ininterrumpido.
...
5.  **Búsqueda en Tiempo Real:** Filtro en el lado del cliente que permite buscar cursos por título o categoría de forma instantánea.
...
7.  **Accesibilidad y Contraste (WCAG Compliance):** La interfaz fue diseñada cuidando los ratios de contraste requeridos por las pautas de accesibilidad, utilizando etiquetas semánticas y atributos `aria-label` en elementos interactivos para garantizar una navegación fluida mediante lectores de pantalla.
8.  **Soporte PWA (Progressive Web App):** Se incluyó un `manifest.json` y meta tags base para dispositivos móviles. El proyecto está estructurado y listo para integrarse fácilmente con **Capacitor** para su empaquetado nativo.

## 📊 Resultados de la Auditoría (Lighthouse)

El proyecto fue sometido a la suite de auditorías de rendimiento, accesibilidad, buenas prácticas y SEO de Google Lighthouse en su entorno de producción (`npm run build` + `npm run preview`), obteniendo una calificación perfecta:

![Resultado de la auditoría de Lighthouse](./src/assets/lighthouse.png)
...
* Soporte para Modo Oscuro. Implementar un sistema de temas global utilizando las clases nativas `dark:` de Tailwind CSS y persistencia en `localStorage` o preferencias del sistema para mejorar la experiencia de lectura nocturna.
